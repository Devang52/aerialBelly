...
export default function Level() {
    ...
    // useEffect(() => {
    //     const filtered = filterDataBySearch(data ?? []);
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (result.isConfirmed) {
                const res = await axios.delete(`${process.env.REACT_APP_BACKEND_BASE_URL}/delete/level/` + id);
                if (res.status === 200) {
                    getProductData();
                    Swal.fire({
                        title: "Deleted!",
                        text: "This level has been deleted.",
                        icon: "success",
                ...
                <div className="page-wrapper page-settings">
                    <div className="content">
                        ...
                        {/* <div className="d-flex justify-content-between">
                            ...
                            <div className="form-group">
                                <Link to="/createlevel"><button className="btn btn-primaryy">create</button></Link>
                            </div>
                        </div> */}
                        <div className="row">
                                        ...
                                        {currentItems.length === 0 ? (
                                            ...
                                            currentItems.map((dataa, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td>{dataa.table_id}</td>
                                                    <td>{dataa.le_name}</td>
                                                    {/* <td>
                                                        <div className="table-actions d-flex">
                                                            <Link
                                                                className="delete-table me-2"
                                                                to={`/editlevel/${dataa.id}`}
                                                            >
