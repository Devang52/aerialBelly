...
export default function EditLevel() {
    ...
    const headleSubmit = async (e) => {
        ...
        try {
            ...
            if (!trimmedEvent.title || !trimmedEvent.description || !trimmedEvent.size) {
                Swal.fire({
                    title: "Please enter data",
                    icon: "warning",
                });
            } else {
                const res = await axios.put(`${process.env.REACT_APP_BACKEND_BASE_URL}/updateproduct/${id}`, data);
