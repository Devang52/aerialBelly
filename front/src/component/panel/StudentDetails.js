...
export default function StudentDetails() {
    ...
    const getStudentData = async () => {
        try {
            const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/getStudentbyID/${id}`);
            if (res.status === 200) {
                setData({
                    name: res.data.data[0].name,
                    email: res.data.data[0].email,
                    address: res.data.data[0].address,
                    de_address: res.data.data[0].de_address,
                    nationality: res.data.data[0].nationality,
                    city: res.data.data[0].city,
                    phone: res.data.data[0].phone,
                    w_number: res.data.data[0].code + ' ' + res.data.data[0].w_number,
                    gender: res.data.data[0].gender,
                    dob: res.data.data[0].dob,
                    health: res.data.data[0].h_issue,
