...
export default function EditWorkshop() {
    ...
    const getWorkshopIdData = async () => {
        try {
            const res = await axios.get(
                `${process.env.REACT_APP_BACKEND_BASE_URL}/getWorkshopbyid/${id}`
            );
            if (res.status === 200) {
                setWorkshop({
                    ...workshop,
                    ...
                    rupees: res.data.data[0].rupees,
                    doller: res.data.data[0].doller,
    ...
    const headleSubmit = async (e) => {
        ...
        try {
            const trimmedWorkshop = {
                title: workshop.title.trim(),
                description: workshop.description.trim(),
                city: workshop.city.trim(),
                location: workshop.location.trim()
            };

            if (!trimmedWorkshop.title || !trimmedWorkshop.description || !trimmedWorkshop.city || !trimmedWorkshop.location || !workshop.rupees || !workshop.doller || !workshop.date) {
                Swal.fire({
                ...
                for (const key in workshop) {
                    formDataObject.append(key, workshop[key])
                }
                const res = await axios.put(`${process.env.REACT_APP_BACKEND_BASE_URL}/updateWorkshop/${id}`, formDataObject);

                if (res.status === 200 && res.data.data) {
                    Swal.fire({
                        icon: "success",
                        title: "Workshop Inserted Successfully",
                        timer: 1500,
                    }).then(() => {
                        navigate("/workshop");
                    });
                    ...
                    Swal.fire({
                        title: "Workshop Inserted failed",
                        icon: "error",
                    });
                }
            }
                                        ...
                                        <div className="col-lg-3">
                                            <div className="form-group">
                                                ...
                                                <input
                                                    ...
                                                    className="form-control"
                                                    onChange={handleInputChange}
                                                    placeholder="Enter Doller"
                                                    required
                                                    value={workshop.doller}
                                                />
                                            </div>
                                        </div>
                                        <div className="col-md-3">
                                            ...
                                            {selectedFile
                                                ...
                                                : (
                                                    <div>
                                                        <div>
                                                            <img src={`${process.env.REACT_APP_BACKEND_BASE_URL}/uploads/${workshop.photo}`} alt="Selected" style={{ width: '250px', height: '300px', objectFit: 'cover', borderRadius: '20px' }} />
                                                        </div>
                                                    </div>
                                                )}
                                        </div>
                                        <div className="btn-path">
                                            <Link to={'/workshop'} className="btn btn-cancel me-3">Back</Link>
