...
export default function PurchasePackages() {
    ...
    const getStudentData = async () => {
        try {
            const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/getOrderItemAll`);
            if (res.status === 200) {
                    ...
                    <div className="content">
                        <nav aria-label="breadcrumb" style={{ '--bs-breadcrumb-divider': 'none' }}>
                            <ol className="breadcrumb">
                                <li className="breadcrumb-item active" aria-current="page">Purchase Packages</li>
                            </ol>
                        </nav>
                        <div className="content-page-header content-page-headersplit">
                            <h5>Purchase Packages</h5>
                        </div>
                                        ...
                                        {currentItems.length === 0 ? (
                                            ...
                                            currentItems.map((dataa, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td>{dataa.table_name === null
                                                        ? dataa.title
                                                        : dataa.table_name.split("").slice(0, 22).join("") + "...."}</td>
                                                    <td>{dataa.level_name === null ? '-' : dataa.level_name}</td>
                                                    <td>{dataa.duration === null ? '-' : dataa.duration}</td>
                                                    <td>{dataa.size === null ? '-' : dataa.size}</td>
                                                    <td style={{ textAlign: '-webkit-center' }}>{dataa.color === null ? '-' : <div style={{ background: dataa.color, height: '35px', width: '35px', borderRadius: '25%' }}></div>}</td>
                                                    <td>{dataa.text === null ? '-' : dataa.text}</td>
                                                    <td>{dataa.quantity === null ? '-' : dataa.quantity}</td>
                                                    <td>{dataa.phone}</td>
                                                    <td>{dataa.w_number}</td>
                                                </tr>
                                            ))
                                        )}
                                    </table>
