...
export default function EditSubscriptionPlan() {
    ...
    const getLevelData = async () => {
        try {
            const res = await axios.get(
                `${process.env.REACT_APP_BACKEND_BASE_URL}/getalllevel`
            );
            if (res.status === 200) {
    ...
    const getTableData = async () => {
        try {
            const res = await axios.get(
                `${process.env.REACT_APP_BACKEND_BASE_URL}/getalltablename`
            );
            if (res.status === 200) {
    ...
    const headleSubmit = async (e) => {
        ...
        try {
            ...
            if (!trimmedData.duration || !data.table_id || !data.rupees || !data.level_id || !data.doller) {
                Swal.fire({
                    title: "Please enter data",
                    icon: "warning",
                });
            } else {
                const res = await axios.post(`${process.env.REACT_APP_BACKEND_BASE_URL}/createsubscription`, data);

                if (res.status === 200 && res.data.data) {
                    Swal.fire({
                        icon: "success",
                        title: "Subscription Plan Inserted Successfully",
                        timer: 1500,
                    }).then(() => {
                    ...
                    Swal.fire({
                        title: "Subscription Plan Inserted failed",
                        icon: "error",
                    });
                }
            }
                                        ...
                                        <div className="col-lg-3">
                                            <div className="form-group">
                                                ...
                                                <input
                                                    ...
                                                    className="form-control"
                                                    onChange={handleInputChange}
                                                    placeholder="Enter Doller Amount"
                                                    required
                                                />
                                            </div>
                                        </div>
                                        <div className="btn-path">
                                            <Link to={'/subscription-plan'} className="btn btn-cancel me-3">Back</Link>
