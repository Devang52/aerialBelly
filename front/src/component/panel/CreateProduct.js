...
export default function CreateProduct() {
    ...
    const addColor = () => {
        if (currentColor && !data.color.includes(currentColor)) {
            ...
            setCurrentColor(''); // Clear input after adding
        }
    };
    ...
    const headleSubmit = async (e) => {
        ...
        try {
            ...
            if (!trimmedEvent.title || !trimmedEvent.description || !trimmedEvent.amount) {
                Swal.fire({
                    title: "Please enter data",
                    icon: "warning",
                });
            } else if (!data.title || !data.description || !data.amount || !data.photo || data.size.length === 0 || data.color.length === 0) {
                Swal.fire({
                ...
                const res = await axios.post(`${process.env.REACT_APP_BACKEND_BASE_URL}/createproduct`, formDataObject);
