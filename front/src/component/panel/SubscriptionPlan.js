    ...
    const getSubsriptionData = async () => {
        try {
            const res = await axios.get(
                `${process.env.REACT_APP_BACKEND_BASE_URL}/getallsubscription`
            );
            if (res.status === 200) {
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (result.isConfirmed) {
                const res = await axios.delete(`${process.env.REACT_APP_BACKEND_BASE_URL}/delete/subscription/` + id);
                if (res.status === 200) {
                    getSubsriptionData();
                    Swal.fire({
                        title: "Deleted!",
                        text: "This Subscription Plan has been deleted.",
                        icon: "success",
                    ...
                    Swal.fire({
                        ...
                        text: "Subscription Plan deletion failed.",
                        icon: "error",
                    });
                }
            }
                    ...
                    <div className="content">
                        ...
                        <div className="d-flex justify-content-between">
                            ...
                            <div className="form-group">
                                <Link to="/create-subscription-plan"><button className="btn btn-primaryy">create</button></Link>
                            </div>
                        </div>
                        <div className="row">
                                        ...
                                        {currentItems.length === 0 ? (
                                            ...
                                            currentItems.map((dataa, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td>{dataa.table_name}</td>
                                                    <td>{dataa.level_name}</td>
                                                    <td>{dataa.duration}</td>
                                                    <td>{dataa.rupees}</td>
                                                    <td>{dataa.doller}</td>
                                                    {/* <td>
                                                        <div className="table-actions d-flex">
                                                            <Link
                                                                ...
                                                                to={`/edit-subscription-plan/${dataa.id}`}
                                                            >
