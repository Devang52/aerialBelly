...
export default function PaymentPanl() {
    ...
    const getStudentData = async () => {
        try {
            const res = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/PaymentHistory`);
            if (res.status === 200) {
                                        ...
                                        {currentItems.length === 0 ? (
                                            ...
                                            currentItems.map((dataa, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td>{new Date(dataa.date).toISOString().split('T')[0]}</td>
                                                    <td>{dataa.total_rupees} </td>
                                                </tr>
                                            ))
                                        )}
                                    </table>
