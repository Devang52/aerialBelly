...
export default function EditEvent() {
    ...
    const getEventIdData = async () => {
        try {
            const res = await axios.get(
                `${process.env.REACT_APP_BACKEND_BASE_URL}/geteventbyid/${id}`
            );
            if (res.status === 200) {
                setEvent({
                    ...event,
    ...
    const headleSubmit = async (e) => {
        ...
        try {
            ...
            if (!trimmedEvent.title || !trimmedEvent.description || !trimmedEvent.address || !event.date) {
                Swal.fire({
                ...
                for (const key in event) {
                    formDataObject.append(key, event[key])
                }
                const res = await axios.put(`${process.env.REACT_APP_BACKEND_BASE_URL}/updateevent/${id}`, formDataObject);
                                            ...
                                            <div className="col-md-3">
                                                ...
                                                {selectedFile
                                                    ...
                                                    : (
                                                        <div>
                                                            <div>
                                                                <img src={`${process.env.REACT_APP_BACKEND_BASE_URL}/uploads/${event.photo}`} alt="Selected" style={{ width: '250px', height: '300px', objectFit: 'cover', borderRadius: '20px' }} />
                                                            </div>
                                                        </div>
                                                    )}
                                            </div>
                                            <div className="btn-path">
                                                <Link to={'/event'} className="btn btn-cancel me-3">Back</Link>
